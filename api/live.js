// ============================================================
//  api/live.js — Air India Dashboard Serverless Proxy
//  Deploy to: /api/live.js in your GitHub repo root
//  All API keys stored as Vercel Environment Variables only
//  Never expose keys in index.html or commit them to GitHub
// ============================================================
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (source === 'fuel') {
      ...
      const jetFuel = fuelData?.response?.data?.[0]?.value ?? 2.65;
      ...
      const brent = brentData?.response?.data?.[0]?.value ?? 82;
      return res.status(200).json({ jetFuel: parseFloat(jetFuel), brent: parseFloat(brent), period: fuelData?.response?.data?.[0]?.period ?? 'latest' });
    }
    ...
    if (source === 'flights') {
      ...
      const data = await r.json();
      const flights = (data.states || []).map(s => ({ callsign: (s[1]||'').trim(), longitude: s[5], latitude: s[6], altitude: s[7], onGround: s[8] }));
    ...
    if (source === 'route-flights') {
      ...
      const IATA_TO_ICAO = { 'DEL':'VIDP','BOM':'VABB','LHR':'EGLL','DXB':'OMDB','JFK':'KJFK','SIN':'WSSS','FRA':'EDDF','SYD':'YSSY','BLR':'VOBL','HYD':'VOHS','MAA':'VOMM','CCU':'VECC','NRT':'RJTT','HKG':'VHHH','CDG':'LFPG' };
      const parts = (route||'DEL-LHR').split('-');
      const depICAO = IATA_TO_ICAO[parts[0]]||'VIDP';
      const arrICAO = IATA_TO_ICAO[parts[1]]||'EGLL';
      const now = Math.floor(Date.now()/1000);
      const auth = Buffer.from(`${user}:${pass}`).toString('base64');
      const r = await fetch(`https://opensky-network.org/api/flights/arrival?airport=${arrICAO}&begin=${now-7200}&end=${now}`, { headers: { Authorization: `Basic ${auth}` } });
      const data = await r.json();
      const relevant = (Array.isArray(data)?data:[]).filter(f=>f.estDepartureAirport===depICAO).slice(0,10);
    ...
    if (source === 'ai') {
      ...
      if (!ANTHROPIC_KEY) return res.status(200).json({ content: [{ type: 'text', text: 'ANTHROPIC_API_KEY not configured in Vercel.' }] });
